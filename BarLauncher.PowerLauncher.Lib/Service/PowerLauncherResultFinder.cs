...
{
    public class PowerLauncherResultFinder : BarLauncherResultFinder
    {
        private IPowerLauncherService PowerLauncherService { get; set; }
        private IHelperService HelperService { get; set; }

        private IApplicationInformationService ApplicationInformationService { get; set; }

        private ISystemService SystemPowerLauncherService { get; set; }

        public PowerLauncherResultFinder(IBarLauncherContextService barLauncherContextService, IPowerLauncherService webAppService, IHelperService helperService, IApplicationInformationService applicationInformationService, ISystemService systemPowerLauncherService) : base(barLauncherContextService)
        {
            PowerLauncherService = webAppService;
            HelperService = helperService;
            ApplicationInformationService = applicationInformationService;
            SystemPowerLauncherService = systemPowerLauncherService;
        }

        public override void Init()
        {
            PowerLauncherService.Init();

            AddCommand("list", "list [PATTERN] [PATTERN] [...]", "List all url matching patterns", GetListResults);
            AddCommand("config", "config [PROFILE] [APP_PATH] [APP_ARGUMENT_PATTERN]", "Configure a new powerlauncher launcher for a profile", GetConfigResults);
        ...
        private void HelpCommand() => SystemPowerLauncherService.OpenUrl(ApplicationInformationService.HomepageUrl);
        ...
        private IEnumerable<BarLauncherResult> GetEditResults(BarLauncherQuery query, int position)
        {
            if (query.SearchTerms.Contains("->"))
            {
                ...
                if (oper == "->")
                {
                    ...
                    var webAppItem = PowerLauncherService.GetUrlInfo(url);
                    ...
                    else
                    {
                        ...
                        GetActionResult(
                            ...
                            () =>
                            {
                                PowerLauncherService.EditPowerLauncherItem(url, newUrl, newKeywords, newProfile);
                            }
                        )
                    };
                    }
            ...
            return PowerLauncherService
                ...
                (
                    ...
                    (
                        ...
                        () =>
                        {
                            PowerLauncherService.StartUrl(item.Url, item.Profile);
                        }
                    )
                );
        }
        ...
        private IEnumerable<BarLauncherResult> GetConfigResults(BarLauncherQuery query, int position)
        {
            if (query.SearchTerms.Length > position)
            {
                ...
                var configuration = PowerLauncherService.GetConfiguration(profile);
                ...
                if (query.SearchTerms.Length > position + 1)
                {
                    ...
                    string argumentsReal = arguments ?? configuration.PowerLauncherArgumentPattern;

                    string title = string.Format("config {0} {1} {2}", profile, launcher, argumentsCommandLine);
                    string subTitle = string.Format("Change {0} powerlauncher launcher to [{1}] and argument to [{2}]", profile, launcher, argumentsReal);
                    ...
                    (
                        ...
                        () =>
                        {
                            PowerLauncherService.UpdateLauncher(launcher, argumentsReal, profile);
                        }
                    );
                } 
                else
                {
                    if (configuration == null)
                    {
                        yield return GetCompletionResultFinal
                            (
                                ...
                                "Create a {0} powerlauncher launcher".FormatWith(profile),
                                () => {
                                    configuration = PowerLauncherService.GetOrCreateConfiguration(profile);
                                    return string.Format("config {0} {1} {2}", profile, configuration.PowerLauncherLauncher, configuration.PowerLauncherArgumentPattern);
                                }
                            );
                    }
                    else
                    {
                        yield return GetCompletionResultFinal
                            (
                                string.Format("config {0} {1} {2}", profile, configuration.PowerLauncherLauncher, configuration.PowerLauncherArgumentPattern),
                                "Change {0} powerlauncher launcher to [{1}] and argument to [{2}]".FormatWith(profile, configuration.PowerLauncherLauncher, configuration.PowerLauncherArgumentPattern),
                                () => string.Format("config {0} {1} {2}", profile, configuration.PowerLauncherLauncher, configuration.PowerLauncherArgumentPattern)
                            );
                    }
                }
            ...
            else
            {
                ...
                foreach (var profile in PowerLauncherService.GetProfiles())
                {
                    var configuration = PowerLauncherService.GetConfiguration(profile);
        ...
        private IEnumerable<BarLauncherResult> GetAddResults(BarLauncherQuery query, int position)
        {
            ...
            if (!string.IsNullOrEmpty(url))
            {
                ...
                var profiles = PowerLauncherService.GetProfiles();
                if (keywords == "")
                {
                    foreach (var existingProfile in profiles)
                    {
                        if (existingProfile.Contains(profile))
                        {
                            ...
                            (
                                ...
                                () =>
                                {
                                    PowerLauncherService.AddPowerLauncherItem(url, keywords, existingProfile);
                                }
                            );
                        }
                    }
        ...
        {
            var webAppItems = PowerLauncherService
                ...
                (
                    item =>
                        ...
                        (
                            ...
                            () => PowerLauncherService.RemoveUrl(urlTyped)
                        )
                        :
                        GetCompletionResultFinal
                        (
        ...
        private IEnumerable<BarLauncherResult> GetOpenResults(BarLauncherQuery query, int position)
        {
            ...
            if (isValid)
            {
                ...
                (
                    ...
                    () =>
                    {
                        PowerLauncherService.StartUrl(url, profile);
                    }
                );
            }
            else
        ...
        private void ExportCommandAction() => PowerLauncherService.Export();
        ...
        private IEnumerable<BarLauncherResult> GetImportResults(BarLauncherQuery query, int position)
        {
            ...
            {
                if (PowerLauncherService.FileExists(filename))
                {
                    yield return GetActionResult
                    (
                        "import {0}".FormatWith(filename),
                        "Import urls from [{0}]".FormatWith(filename),
                        () => PowerLauncherService.Import(filename)
                    );
                }
                else
                {
